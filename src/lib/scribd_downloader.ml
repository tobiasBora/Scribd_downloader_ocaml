...
let map_file f file_in file_out =
  F.seq_of_file file_in
  |> S.map f
  |> F.write_in_file (file_out ^ ".tmp");
...
let run_command ?(prog="") pr args outfile =
  ...
  >|= (fun str -> S.singleton str |> F.write_in_file outfile)
...
let filter_nodes f reg_str_begin reg_str_classic_begin reg_str_end filename_in filename_out =
  ...
  F.seq_of_file filename_in
  |> S.filter (f reg_b reg_m reg_e)
  |> F.write_in_file (filename_out ^ ".tmp");
...
let step_1_download_page pr conf url =
  F.write_in_file
    "phantom_step1.js"
    (S.singleton ("var page = require('webpage').create();
url = \"" ^ url ^ "\"
...
let step_3_get_pages_size conf =
  ...
  let get_width line =
    ...
    with _ -> None
  in
  let pages_size_seq = F.seq_of_file "page.html"
		       |> S.filter_map get_width |> S.persistent in
  let n_tot = S.length pages_size_seq in
  let max_h = pages_size_seq |> S.map (fun (h,w) -> h) |> S.max |> CCOpt.get_exn in
  let max_w = pages_size_seq |> S.map (fun (h,w) -> w) |> S.max |> CCOpt.get_exn in
  (n_tot, pages_size_seq, (max_h, max_w))


(* TODO : do it with sequence *)
(** Usefull in debug mode to check that pages are well aligned *)
let color_pages nb_pages =
  let r () = Random.int 256 in
  S.(1 -- nb_pages)
  |> S.map (fun i -> Printf.sprintf
		       "#outer_page_%d {background-color: rgb(%d,%d,%d);}"
		       i (r ()) (r ()) (r ()))
  |> S.cons "<style>\n"
  |> (fun seq -> S.snoc seq "</style>\n")
  |> F.write_in_file ~mode:[Open_wronly; Open_text; Open_append] "page.html"
...
let step_6_aux_get_individual_png pr conf nb_page_to_convert page_size_stack next_name0 =
  let nb_pages = nb_page_to_convert in
  let current_h = ref 0 in
  let rec loop i next_name =
    if i = 0 then Lwt.return next_name
    else 
      let (h,w) = Stack.pop page_size_stack in
      let h_z = h * conf.zoom
...
let step_6_get_group_page pr pourcent conf pages_size_seq max_sizes =
  let pages_size_stack = Stack.create () in
  S.to_stack pages_size_stack pages_size_seq;
  let n_tot = Stack.length pages_size_stack in
  let rec loop next_name =
    if Stack.is_empty pages_size_stack then
      Lwt.return_unit
    else
      begin
	let nr = Stack.length pages_size_stack in
	...
	>>= fun _ ->
	step_6_aux_get_individual_png pr conf nd pages_size_stack next_name
	>>= loop
      end
  in
  loop 0
...
let step_7_generate_pdf pr conf =
  let file_list =
    ...
    |> S.of_list
    |> S.sort
    |> S.filter (fun str -> match_reg (Str.regexp "0_.*\\.pdf$") str)
    |> S.to_list
  in
  let pdf_list =
    file_list
